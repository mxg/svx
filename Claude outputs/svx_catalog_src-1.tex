...
\section{msg macros --- Convenience Message Macros}
\noindent\small\textit{File: \texttt{src/utils/msg\_macros.svh}}\par

Preprocessor convenience macros that forward directly to the \texttt{msg} class static functions. Using the macros avoids writing \texttt{msg::} at every call site and keeps diagnostic output consistent across the library.

\subsection*{\texttt{msg macros}}
\noindent\small\textit{macros}\par

Four single-statement macros, one per severity level. Each expands to the corresponding \texttt{msg} static-function call.
...
\bottomrule
\endfoot
\texttt{\`{}info\_msg(id,\ txt)} & void & --- & Expands to \textbackslash{}texttt\{msg::info(id,txt)\}. \\
\texttt{\`{}warning\_msg(id,\ txt)} & void & --- & Expands to \textbackslash{}texttt\{msg::warning(id,txt)\}. \\
\texttt{\`{}error\_msg(id,\ txt)} & void & --- & Expands to \textbackslash{}texttt\{msg::error(id,txt)\}. \\
\texttt{\`{}fatal\_msg(id,\ txt)} & void & --- & Expands to \textbackslash{}texttt\{msg::fatal(id,txt)\}. \\
\end{xltabular}
