...
\texttt{set\_bounds\_lock} & void & \texttt{bit lock} & When lock=1 the bounds cannot be changed; further calls to set\_bounds* are ignored. \\
...
\texttt{insert\_space} & \texttt{bit} & \texttt{mem\_space\_t child} & Insert child into this space; returns 1 on success, 0 on type violation. \\
\texttt{get\_error} & \texttt{string} & --- & Return the last error message, if any. \\
